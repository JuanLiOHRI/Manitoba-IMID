Spca_plot <- function(res, nharm = 2, titlestr = NULL) {
  ...
  harmvarmxfd  <- res$harmvarmxfd
  varpropvarmx <- res$varpropvarmx
    ...
    if (nharm == 2) {
      ...
      fig <- plot_ly(df_harmmat, x = ~PC1, y = ~PC2, type = 'scatter', 
        ...
        layout(title = titlestr, 
               xaxis = list(title = paste0("PC.1 (", round(varpropvarmx[1] * 100, 1), "%)")), 
               yaxis = list(title = paste0("PC.2 (", round(varpropvarmx[2] * 100, 1), "%)")), 
               showlegend = FALSE) 
      return(fig)
    } else {
      #   -----------------------  three dimensions -----------------------
      fig <- plot_ly(df_harmmat, x = ~PC1, y = ~PC2, z = ~PC3, type = 'scatter3d', 
        ...
        add_trace(x = df_Q$PC1[5], 
                  ...
                  mode = "text+lines") %>% 
        layout(title = titlestr, 
               scene = list(xaxis = list(title = paste0("PC.1 (", round(varpropvarmx[1] * 100, 1), "%)")), 
                            yaxis = list(title = paste0("PC.2 (", round(varpropvarmx[2] * 100, 1), "%)")),
                            zaxis = list(title = paste0("PC.3 (", round(varpropvarmx[3] * 100, 1), "%)"))),
               showlegend = FALSE)
      return(fig)
    }
  } else {
    stop("The current Spca.plot only works with 2 or 3 dimension.")
  }
